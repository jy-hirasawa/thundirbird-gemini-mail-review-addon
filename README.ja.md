...
- 🔒 **セキュア**: API キーは Thunderbird にローカル保存
...
4. 歯車アイコン ⚙️ をクリックして **アドオンをデバッグ** を選択
5. **一時的なアドオンを読み込む** をクリック
6. アドオンディレクトリに移動して `manifest.json` ファイルを選択
...
- **スマート検出**: メールコンテンツが変更されると自動的に検出し、新しい分析をリクエスト

**キャッシングの仕組み:**
- 各メールは件名、受信者、本文内容のユニークなハッシュで識別されます
- 同じメールを再度分析すると、キャッシュされたレスポンスが表示されます
- メールを編集して再度チェックすると、新しいメールとして扱われ、新たに分析されます
...
このアドオンは、分析のためにメールコンテンツを Google の Gemini API に送信します。メールは [Google のプライバシーポリシー](https://policies.google.com/privacy?hl=ja)に従って処理されます。API キーは Thunderbird プロファイルにローカル保存され、Google の API エンドポイント以外の第三者には送信されません。
