...
2. アドオンの設定:
   ...
   - （オプション）Gemini がメールを分析する方法をカスタマイズするためにカスタムプロンプトテンプレートを追加
     ...
     - **多言語対応**: 任意の言語でプロンプトを記述すると、その言語で分析結果が得られます
     - 例（日本語）: "以下のメール本文が、取引先・顧客など会社宛てのメールとして、敬語や言い回しが適切か、失礼・不自然・誤解を招く表現がないか、ビジネスメールとして十分にフォーマルかを確認してください。問題点があれば、理由とあわせて修正案を提示してください。"
     - 例（英語）: "Review this email for business communication. Check if the language is polite, appropriate for clients, and sufficiently formal. Flag any inappropriate, unnatural, or misleading expressions."
...
- 詳細は [SECURITY.md](doc/ja/SECURITY.md) をご覧ください
