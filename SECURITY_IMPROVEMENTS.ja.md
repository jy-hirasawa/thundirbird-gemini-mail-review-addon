...
1. **APIキー** - プレーンテキストで保存（browser.storage.localの制限）
2. **APIエンドポイント** - 検証済みのHTTPS URLのみ
3. **カスタムプロンプトテンプレート** - サニタイズ済み、長さ制限あり
4. **メールキャッシュ** - 件名、受信者、本文、分析結果（最大50件、自動期限切れ）
5. **最後にチェックしたハッシュ** - 変更検出用のSHA-256ハッシュ（最大20件）
